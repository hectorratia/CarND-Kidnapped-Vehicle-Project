...
//******How to do it using range & bearing... programmed this because the header file wrongly documents the standard deviations as range and bearing.
//      double range_o= pow(pow(l_x-x_p,2)+pow(l_y-y_p,2),0.5);
//      double bearing_o= atan2(l_y-y_p,l_x-x_p);       
//      double range_p= pow(pow(o_x_map-x_p,2)+pow(o_y_map-y_p,2),0.5);
//      double bearing_p= atan2(o_y_map-y_p,o_x_map-x_p);       
//      weight = weight * exp(-0.5 * (pow((range_o - range_p) / std_landmark[0],2) + pow((bearing_o - bearing_p) / std_landmark[1],2))) / (2*M_PI*std_landmark[0]*std_landmark[1]);
